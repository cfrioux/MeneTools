...
def get_intersection_of_paths(draft, seeds, targets):

    draftfact  = String2TermSet('draft("draft")')
    instance   = TermSet(draft.union(draftfact).union(targets).union(seeds))
...
def get_optimal_solutions_cof(draft, seeds, targets, cofactors, optimum, weighted, nmodels=0):
    ...
    if weighted:
        ...
        prg = [cof_prg, draft_f, seed_f, targets_f, cofactors_f]
    options = str(nmodels)+' --configuration jumpy --opt-strategy=usc,5 --opt-mode=optN --opt-bound=' + optimum
    solver = Gringo4Clasp(clasp_options=options)
    models = solver.run(prg, collapseTerms=True, collapseAtoms=False)
    os.unlink(draft_f)
    os.unlink(seed_f)
